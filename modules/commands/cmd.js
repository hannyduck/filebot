module.exports.config = {
    ...
    version: "1.0.0",
    hasPermssion: 2,
    credits: "Mirai Team", //mod bởi nguyễn đức tài
...
const loadCommand = function ({ moduleList, threadID, messageID }) {
  ...
  //api ảnh load thành công
  const attachment = (await global.nodemodule["axios"]({
            url: (await global.nodemodule["axios"]('https://dtai2006.nguyenductai206.repl.co/cmd')).data.url,
            method: "GET",
            responseType: "stream"
        })).data;
    const logger = require(mainPath + '/utils/log');
  //api ảnh lỗi
  const attachment1 = (await global.nodemodule["axios"]({
            url: (await global.nodemodule["axios"]('https://dtai2006.nguyenductai206.repl.co/loi')).data.url,
            method: "GET",
            responseType: "stream"
        })).data;
  var errorList = [];
 ...
 'tiến hành cài đặt...', 'warn');
    ...
    if (errorList.length != 0) api.sendMessage({body: `» Lỗi tải lệnh! không thể khởi chạy lệnh do có đoạn code bị lỗi! Chỗ bị lỗi là:  ${errorList.join(' ')}`, attachment1}, threadID, messageID);
    api.sendMessage({body: `» Đã tải thành công ${(moduleList.length - errorList.length)} lệnh`, attachment}, threadID, messageID) 
...
const unloadModule = function ({ moduleList, threadID, messageID }) {
  ...
  const attachment = (await global.nodemodule["axios"]({
    ...
    return api.sendMessage({body: `» Đã hủy thành công ${moduleList.length} lệnh`, attachment}, threadID, messageID);
}

module.exports.run = function ({ event, args, api }) {
  
    if (event.senderID != 100072782650649) return api.sendMessage(`Ai cho dùng`, event.threadID, event.messageID)
    ...
    switch (args[0]) {
      ...
      let commands = client.commands.values();
		  let infoCommand = "";
			api.sendMessage({body: `» Hiện tại đang có ${client.commands.size} lệnh có thể sử dụng!  ${infoCommand}`, attachment}, event.threadID, event.messageID);
      break;
		}
        case "load":
        case "l": {
            if (moduleList.length == 0) return api.sendMessage({body: `» Tên module không được để trống!`, attachment}, threadID, messageID);
      ...
      case "-i": {
            ...
            if (!command) return api.sendMessage({body: `» Module bạn nhập không tồn tại!`, attachment}, threadID, messageID);
