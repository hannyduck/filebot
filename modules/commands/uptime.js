module.exports.config = {
	...
	description: "Random ảnh anime api - uptime",
...
const time = process.uptime() ,
  ...
  const fs = require('fs-extra');
   if (!fs.existsSync(__dirname +
        ...
        fs.writeFileSync(__dirname + `/bot/UTM-Avo.ttf`, Buffer.from(getfont, "utf-8"));
      }
         if (!fs.existsSync(__dirname +
      `/bot/phenomicon.ttf`)) {
      let getfont2 = (await axios.get(`https://github.com/hanakuUwU/font/raw/main/phenomicon.ttf`, { responseType: "arraybuffer" })).data;
      fs.writeFileSync(__dirname + `/bot/phenomicon.ttf`, Buffer.from(getfont2, "utf-8"));
    };
  if (!fs.existsSync(__dirname +
      `/bot/CaviarDreams.ttf`)) {
      let getfont3 = (await axios.get(`https://github.com/hanakuUwU/font/raw/main/CaviarDreams.ttf`, { responseType: "arraybuffer" })).data;
      fs.writeFileSync(__dirname + `/bot/CaviarDreams.ttf`, Buffer.from(getfont3, "utf-8"));
    };
...
let l1 = await loadImage(pathAva);
  ...
  ctx.drawImage(l1, 800, -160, 1100, 1100);
     registerFont(__dirname + `/bot/phenomicon.ttf`, {
      family: "phenomicon"
    });
    ctx.textAlign = "start";
    ...
    ctx.fillText("Mãi keo?", 95, 340);
    ctx.beginPath();
  ////////////////////////////////////////
   registerFont(__dirname + `/bot/UTM-Avo.ttf`, {
      family: "UTM"
    });
    ctx.textAlign = "start";
    ctx.font = "70px UTM";
...
registerFont(__dirname + `/bot/CaviarDreams.ttf`, {
    ...
    ctx.textAlign = "start";
    ctx.font = "45px time";
    ctx.fillText("@" + "minhtien", 250, 515)
    ctx.fillText("@" + "mtien.0703", 250, 575)
