...
module.exports.run = async ({ api, event, args }) => {
  ...
  switch (type.toLowerCase()) {
    ...
    case "info":
      ...
      try {
        axios.get(encodeURI(`https://apidungkonuser.dungkonuwu.repl.co/tiktok?info=${keyword}`)).then(async (res) => {
  ...
  fs.writeFileSync(path, Buffer.from(getimg, "utf-8"));
    ...
    case '-s':
      ...
      axios.get(`https://apidungkonuser.dungkonuwu.repl.co/tiktok?search=${encodeURI(search)}`).then(async res => {
    ...
    case "video":
      try {   
        const res = await axios.get(`https://apidungkonuser.dungkonuwu.repl.co/tiktok?video=${keyword}`);
    ...
    case "-tr":
    case "trending":
      axios.get(`https://apidungkonuser.dungkonuwu.repl.co/tiktok?trending=VN`).then(response_api => {
