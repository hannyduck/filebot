...
module.exports.config = {
    ...
    version: "1.0.0",
    hasPermssion: 0,
    credits: "Thiệu Trung Kiên", // được fix bởi nguyễn đức tài trùm cuti
...
module.exports.downLoad = function (url, type, api, event) {
    ...
    this.getLink(url).then(res => {
        ...
        axios({
            ...
            if (fs.statSync(path).size / 1024 / 1024 > 25) return api.sendMessage("File lớn vãi cả lồn đút buồi bố mày gửi cho nữa lần sau,lấy file nó nhẹ thôi nha con chó", event.threadID, () => fs.unlinkSync(path), event.messageID);
